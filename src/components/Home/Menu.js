...
class Menu extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            priceListItems: props.items.edges,
    ...
    handleItems = (category) => {
        ...
        if (category === "all") {
            this.setState(() => {
                return { priceListItems: tempItems }
            })
        }
        else {
            let items = tempItems.filter(({ node }) => node.category === category);
            this.setState(() => {
                return { priceListItems: items };
            })
        }
    }
                                ...
                                <div className="row mb-5">
                                    <div className="col-10 mx-auto text-center">
                                        {this.state.categories.map((category, index) => {
                                            return (<button type="button" key={index} className="btn btn-whiteish text-capitalize m-1" onClick={() => { this.handleItems(category) }}>{category}</button>)
                                        })}
                                    </div>
                                </div>
                                {/* Items */}
